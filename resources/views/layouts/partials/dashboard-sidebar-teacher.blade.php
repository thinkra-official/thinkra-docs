@php
    ...
    $contentRoutes = ['teacher.courses.show', 'teacher.lessons.*', 'teacher.sections.*', 'teacher.sub-sections.*'];
@endphp
...
<aside
        ...
        <a href="{{ route('teacher.dashboard') }}"
           ...
           @click="sidebarOpen = false">
            ...
            <div>
                ...
                <span class="text-xs text-white/50">بوابة الأستاذ</span>
            </div>
        </a>
    </div>
    ...
    <nav class="flex-1 p-4 space-y-1 overflow-y-auto relative z-10">
        <a href="{{ route('teacher.dashboard') }}"
           class="thinkra-nav-link {{ $isActive('teacher.dashboard') }}"
           @click="sidebarOpen = false">
        ...
        <a href="{{ route('teacher.courses.index') }}"
           class="thinkra-nav-link {{ $isActive('teacher.courses.index') }}"
           @click="sidebarOpen = false">
    ...
    <form method="POST" action="{{ route('teacher.logout') }}" class="p-4 border-t border-white/10 relative z-10">
        @csrf
