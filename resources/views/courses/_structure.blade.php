...
@endphp
<div class="thinkra-card mb-6 overflow-hidden" x-data="{ openSub: false, addSectionLesson: false, editSection: false }">
    <div class="bg-thinkra-navy/5 px-4 py-3 flex flex-wrap justify-between items-center gap-2 border-b">
        ...
        <div class="flex items-center gap-2 text-xs shrink-0">
            ...
            @if($canEditContent)
            <button @click="addSectionLesson = !addSectionLesson" type="button" class="text-thinkra-purple font-semibold">+ درس</button>
            @endif
    ...
    @if($canEditContent)
    <form x-show="addSectionLesson" x-cloak method="POST" action="{{ $routeSectionLesson('store', ['course' => $course, 'section' => $section]) }}" class="px-4 py-3 border-b bg-violet-50/40 flex gap-2">
        @csrf
        <input name="title" required placeholder="عنوان الدرس" class="flex-1 rounded-lg border px-3 py-2 text-sm">
        <button type="submit" class="thinkra-btn-primary px-3 py-2 text-sm">إنشاء درس</button>
    </form>
    @endif
    ...
    @endphp
    <div class="border-t border-slate-100" x-data="{ addLesson: false, editSub: false }">
        <div class="px-4 py-2.5 bg-amber-50/50 flex flex-wrap justify-between items-center gap-2">
            ...
            <div class="flex items-center gap-2 text-xs shrink-0">
                ...
                @endif
                @if($canEditContent)
                <button @click="addLesson = !addLesson" type="button" class="text-thinkra-purple font-semibold">+ درس</button>
                @endif
            </div>
        </div>

        @if($canEditContent)
        <form x-show="addLesson" x-cloak method="POST" action="{{ $routeSubLesson('store', ['course' => $course, 'section' => $section, 'subSection' => $subSection]) }}" class="px-4 py-3 border-b flex gap-2 mr-4">
            @csrf
            <input name="title" required placeholder="عنوان الدرس" class="flex-1 rounded-lg border px-3 py-2 text-sm">
            <button type="submit" class="thinkra-btn-primary px-3 py-2 text-sm">إنشاء</button>
        </form>
        @endif
